...
from sklearn.linear_model import LogisticRegression
model=LogisticRegression()
a=np.array(x_train.iloc[:,[15,16,17,18,19]])
b=np.array(y_train)
model.fit(a,b)
...
coef=model.coef_
print(coef)
model.score(x_test.iloc[:,[15,16,17,18,19]],y_test)
...
export_graphviz(model,out_file="tree.dot",feature_names=['marital','default','contact'],impurity=False,filled=True)
with open("tree.dot") as f :
    dot_graph=f.read()
    graph=export_graphviz(f)
display(graph)
